...
for modelChoice in MODEL_CHOICES:
    _, targetSize, isCaffeModel, decode_predictions, preprocess_input = AttackModels.getModel(modelChoice)
    fmodel = resnet50(pretrained=True)
    fmodel.train(False)
    freeze_model(fmodel)
#     fmodel.eval()
...
#             fmodel = PyTorchModel(model, bounds=(0, 255), num_classes=1000)
            batcher = Batcher(targetSize=targetSize, preProcessInput=preprocess, pathOfImages=PATH_TO_TRAINING_IMAGES,
